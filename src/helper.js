...
export const positionIsSafe = (multArr, position, move) => {
  ...
  const rightUpValue = (position.y-1 > -1 && position.x+1 < multArrLength.x) ? multArr[position.y-1][position.x+1] : 0;
  const rightDownValue = (position.y+1 < multArrLength.x && position.x+1 < multArrLength.x) ? multArr[position.y+1][position.x+1] : 0;
...
export const getFirstPosition = (multArr) => {
  const highestNumbersRows = multArr.map(function(y){ return Math.max.apply(Math, y); });
...
export const getNextPosition = (multArr, lastMove, position) => {
  ...
  if (lastMove === RIGHT_UP || lastMove === RIGHT_DOWN || lastMove === '') {
    if (rightPosition.isValid(multArr)) {
      nextPositions.push({ nextPosition: rightPosition, value: multArr[rightPosition.y][rightPosition.x], move: RIGHT });
    }
  }

  if (lastMove === RIGHT_UP || lastMove === RIGHT || lastMove === '') {
    if (rightDownPosition.isValid(multArr)) {
      nextPositions.push({ nextPosition: rightDownPosition, value: multArr[rightDownPosition.y][rightDownPosition.x], move: RIGHT_DOWN });
    }
  }

  if (lastMove === RIGHT_DOWN || lastMove === RIGHT || lastMove === '') {
    if (rightUpPosition.isValid(multArr)) {
      nextPositions.push({ nextPosition: rightUpPosition, value: multArr[rightUpPosition.y][rightUpPosition.x], move: RIGHT_UP });
    }
  }
